...
class CreateAccountView(SalesAccessRequiredMixin, LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.request.user.role == "ADMIN" or self.request.user.is_superuser:
            ...
            self.users = User.objects.filter(
                role="ADMIN", company=request.company
            ).order_by("email")
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update({"request_obj": self.request})
        # if self.request.user.role != "ADMIN" and not self.request.user.is_superuser:
        #     kwargs.update({"request_user": self.request.user})
        return kwargs
...
class AccountDetailView(SalesAccessRequiredMixin, LoginRequiredMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if account.company != request.company:
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.role != "ADMIN" and not self.request.user.is_superuser:
            if not (
                ...
                or (self.request.user in account_record.assigned_to.all())
            ):
                raise PermissionDenied
...
class AddCommentView(LoginRequiredMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            ...
            or request.user in self.account.assigned_to.all()
        ):
            form = self.get_form()
            if form.is_valid():
                return self.form_valid(form)
