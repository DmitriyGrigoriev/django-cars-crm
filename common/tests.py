...
from common.models import User, Document, Attachments, Company
...
class ObjectsCreation(object):
    def setUp(self):
        ...
        self.comment = Comment.objects.create(
            comment="comment", user=self.user, commented_by=self.user
        )
...
class PasswordChangeTestCase(ObjectsCreation, TestCase):
    ...
    def test_password_invalid(self):
        self.client.login(username="johndoe@admin.com", password="password")
        url = "/change-password/"
        data = {"CurrentPassword": " ", "Newpassword": "test123", "confirm": " "}
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, 200)
    ...
    def change_passsword_by_admin(self):
        response = self.client.post(
            ...
            {"useer_id": self.user.id, "new_passwoord": "password"},
        )
        self.assertEqual(response.status_code, 302)
...
class LoginViewTestCase(ObjectsCreation, TestCase):
    ...
    def test_login_post(self):
        ...
        response = self.client.post(
            reverse("common:login"), data, HTTP_HOST="test.bottlecrm.com"
        )
        self.assertEqual(response.status_code, 302)
...
class CommentTestCase(ObjectsCreation, TestCase):
    def test_comment_add(self):
        response = self.client.post("/comment/add/", {"userid": self.user.id})
        self.assertEqual(response.status_code, 200)

    def test_comment_edit(self):
        response = self.client.post(
            reverse("common:edit_comment", kwargs={"pk": self.comment.id})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_comment_delete(self):
        response = self.client.post("/comment/remove/", {"comment_id": self.comment.id})
        self.assertEqual(response.status_code, 200)

    def test_form_valid(self):
        response = self.client.post(
            "/comment/add/", {"userid": self.user.id, "comment": "hello"}
        )
        self.assertEqual(response.status_code, 200)
...
class DocumentTestCase(ObjectsCreation, TestCase):
    ...
    def test_document_edit(self):
        ...
        response = self.client.get(
            reverse("common:edit_doc", kwargs={"pk": self.document.id}), data
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_document_delete(self):
        response = self.client.get(
            reverse("common:remove_doc", kwargs={"pk": self.document.id})
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_document_list_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        response = self.client.post(
            reverse("common:doc_list"),
            {"doc_name": "doc", "status": "active", "shared_to": str(self.user.id)},
        )
        self.assertEqual(response.status_code, 200)
...
class TestChangeUserStatus(ObjectsCreation, TestCase):
    def test_change_user_status(self):
        response = self.client.get(
            reverse("common:change_user_status", args=(self.user2.id,))
        )
        self.assertEqual(response.status_code, 302)
...
class TestViewApiSettings(ObjectsCreation, TestCase):
    def test_view_api_settings(self):
        ...
        self.api_settings = APISettings.objects.create(
            title="api key", apikey="a45fds54fds54", website="https://micropyramid.com"
        )
...
class TestChangePasswordByAdmin(ObjectsCreation, TestCase):
    def test_change_password_by_admin(self):
        response = self.client.post(
            reverse("common:change_passsword_by_admin"),
...
class TestCommentDelete(ObjectsCreation, TestCase):
    def test_comment_delete_form(self):
        ...
        self.comment_user = Comment.objects.create(
            comment="test comment", commented_by=self.user_admin, lead=self.lead
        )
        response = self.client.post(
            reverse("common:remove_comment"), {"comment_id": self.comment_user.id}
        )
        self.assertJSONEqual(
            force_str(response.content), {"cid": str(self.comment_user.id)}
        )
...
class TestCommentEditErrors(ObjectsCreation, TestCase):
    def test_comment_edit_form(self):
        ...
        self.comment_user = Comment.objects.create(
            comment="comment test", commented_by=self.user_admin, lead=self.lead
        )
        response = self.client.post(
            reverse("common:edit_comment", args=(self.comment_user.id,)),
            {"pk": self.comment_user.id, "comment": ""},
        )
        self.assertJSONEqual(
            force_str(response.content), {"error": ["This field is required."]}
        )
...
class TestUserUpdate(ObjectsCreation, TestCase):
    def test_user_update(self):
        ...
        self.assertEqual(
            force_str(response.content),
            json.dumps({"success_url": reverse("common:users_list"), "error": False}),
        )
    ...
    def test_user_update_permissions(self):
        ...
        self.client.login(username="janeDoeCommon@user.com", password="password")
        response = self.client.post(
            reverse("common:edit_user", args=(self.user_obj.id,)),
            {
                "first_name": "joe",
                "last_name": "doe",
                "username": "joe d",
        ...
        self.assertJSONEqual(
            force_str(response.content), json.dumps({"error_403": True, "error": True})
        )
...
class TestUserCreationView(ObjectsCreation, TestCase):
    def test_user_creation_view(self):
        response = self.client.post(
            reverse("common:create_user"), {}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
        self.assertEqual(
            force_str(response.content),
            json.dumps(
        ...
        self.assertEqual(
            force_str(response.content),
            json.dumps({"success_url": "/users/list/", "error": False}),
        )
    ...
    def test_add_api_settings(self):
        ...
        self.assertTrue("error" in str(response.content))
        response = self.client.post(
            reverse("common:add_api_settings"),
            {
                "title": "api title",
        ...
        self.assertJSONEqual(
            force_str(response.content),
            json.dumps(
                {"success_url": reverse("common:add_api_settings"), "error": False}
            ),
        )
            ...
            json.dumps(
                {
                    ...
                    "errors": {
                        ...
                        "website": ["Please provide valid schema"],
                    },
                }
            ),
        )
    ...
    def test_update_api_settings(self):
        self.api_settings_update = APISettings.objects.create(
            title="api key update", apikey="asdfasd", website="https://example.com"
        )
        response = self.client.post(
...
class TestUserFormValidation(ObjectsCreation, TestCase):
    ...
    def test_document_unique_title(self):
        ...
        response = self.client.post(
            reverse("common:edit_doc", args=(self.document.id,)), data
        )
        json_response = {
            "error": True,
            "errors": {"title": ["Document with this Title already exists"]},
        }
        self.assertJSONEqual(force_str(response.content), json.dumps(json_response))
        ...
        json_response = {
            ...
            "errors": {
                ...
                "document_file": ["This field is required."],
            },
        }
        self.assertJSONEqual(force_str(response.content), json.dumps(json_response))
    ...
    def test_api_setting_form(self):
        ...
        self.assertJSONEqual(force_str(response.content), json_response)
    ...
    def test_get_complete_address(self):
        ...
        Address_obj_street_address_line = Address.objects.create(
            street="street", address_line="address_line"
        )
        Address_obj_street_state = Address.objects.create(
            street="street", state="state"
        )
        Address_obj_street_postcode = Address.objects.create(
            street="street", postcode="postcode"
        )
        Address_obj_street_country = Address.objects.create(
            street="street", country="IN"
        )
        Address_obj_city_address_line = Address.objects.create(
            city="city", address_line="address_line"
        )
        Address_obj_city_state = Address.objects.create(city="city", state="state")
        Address_obj_city_postcode = Address.objects.create(
            city="city", postcode="postcode"
        )
        Address_obj_city_country = Address.objects.create(city="city", country="IN")
        Address_obj_address_line_state = Address.objects.create(
            address_line="address_line", state="state"
        )
        Address_obj_address_line_postcode = Address.objects.create(
            address_line="address_line", postcode="postcode"
        )
        Address_obj_address_line_country = Address.objects.create(
            address_line="address_line", country="IN"
        )
        Address_obj_state_postcode = Address.objects.create(
            state="state", postcode="postcode"
        )
        Address_obj_state_country = Address.objects.create(state="state", country="IN")
        Address_obj_postcode_country = Address.objects.create(
            postcode="postcode", country="IN"
        )
        Address_obj_street_city_address_line = Address.objects.create(
            street="street", city="city", address_line="address_line"
        )
        Address_obj_street_city_state = Address.objects.create(
            street="street", city="city", state="state"
        )
        Address_obj_street_city_postcode = Address.objects.create(
            street="street", city="city", postcode="postcode"
        )
        Address_obj_street_city_country = Address.objects.create(
            street="street", city="city", country="IN"
        )
        Address_obj_street_address_line_state = Address.objects.create(
            street="street", address_line="address_line", state="state"
        )
        Address_obj_street_address_line_postcode = Address.objects.create(
            street="street", address_line="address_line", postcode="postcode"
        )
        Address_obj_street_address_line_country = Address.objects.create(
            street="street", address_line="address_line", country="IN"
        )
        Address_obj_street_state_postcode = Address.objects.create(
            street="street", state="state", postcode="postcode"
        )
        Address_obj_street_state_country = Address.objects.create(
            street="street", state="state", country="IN"
        )
        Address_obj_street_postcode_country = Address.objects.create(
            street="street", postcode="postcode", country="IN"
        )
        Address_obj_city_address_line_state = Address.objects.create(
            city="city", address_line="address_line", state="state"
        )
        Address_obj_city_address_line_postcode = Address.objects.create(
            city="city", address_line="address_line", postcode="postcode"
        )
        Address_obj_city_address_line_country = Address.objects.create(
            city="city", address_line="address_line", country="IN"
        )
        Address_obj_city_state_postcode = Address.objects.create(
            city="city", state="state", postcode="postcode"
        )
        Address_obj_city_state_country = Address.objects.create(
            city="city", state="state", country="IN"
        )
        Address_obj_city_postcode_country = Address.objects.create(
            city="city", postcode="postcode", country="IN"
        )
        Address_obj_address_line_state_postcode = Address.objects.create(
            address_line="address_line", state="state", postcode="postcode"
        )
        Address_obj_address_line_state_country = Address.objects.create(
            address_line="address_line", state="state", country="IN"
        )
        Address_obj_address_line_postcode_country = Address.objects.create(
            address_line="address_line", postcode="postcode", country="IN"
        )
        Address_obj_state_postcode_country = Address.objects.create(
            state="state", postcode="postcode", country="IN"
        )
        Address_obj_street_city_address_line_state = Address.objects.create(
            street="street", city="city", address_line="address_line", state="state"
        )
        Address_obj_street_city_address_line_postcode = Address.objects.create(
            street="street",
            city="city",
            address_line="address_line",
            postcode="postcode",
        )
        Address_obj_street_city_address_line_country = Address.objects.create(
            street="street", city="city", address_line="address_line", country="IN"
        )
        Address_obj_street_city_state_postcode = Address.objects.create(
            street="street", city="city", state="state", postcode="postcode"
        )
        Address_obj_street_city_state_country = Address.objects.create(
            street="street", city="city", state="state", country="IN"
        )
        Address_obj_street_city_postcode_country = Address.objects.create(
            street="street", city="city", postcode="postcode", country="IN"
        )
        Address_obj_street_address_line_state_postcode = Address.objects.create(
            street="street",
            address_line="address_line",
            state="state",
            postcode="postcode",
        )
        Address_obj_street_address_line_state_country = Address.objects.create(
            street="street", address_line="address_line", state="state", country="IN"
        )
        Address_obj_street_address_line_postcode_country = Address.objects.create(
            street="street",
            address_line="address_line",
            postcode="postcode",
            country="IN",
        )
        Address_obj_street_state_postcode_country = Address.objects.create(
            street="street", state="state", postcode="postcode", country="IN"
        )
        Address_obj_city_address_line_state_postcode = Address.objects.create(
            city="city", address_line="address_line", state="state", postcode="postcode"
        )
        Address_obj_city_address_line_state_country = Address.objects.create(
            city="city", address_line="address_line", state="state", country="IN"
        )
        Address_obj_city_address_line_postcode_country = Address.objects.create(
            city="city", address_line="address_line", postcode="postcode", country="IN"
        )
        Address_obj_city_state_postcode_country = Address.objects.create(
            city="city", state="state", postcode="postcode", country="IN"
        )
        Address_obj_address_line_state_postcode_country = Address.objects.create(
            address_line="address_line",
            state="state",
            postcode="postcode",
            country="IN",
        )
        Address_obj_street_city_address_line_state_postcode = Address.objects.create(
            street="street",
            city="city",
            address_line="address_line",
            state="state",
        ...
        Address_obj_street_city_address_line_state_country = Address.objects.create(
            street="street",
            city="city",
            address_line="address_line",
            state="state",
        ...
        Address_obj_street_city_address_line_postcode_country = Address.objects.create(
            street="street",
            city="city",
            address_line="address_line",
            postcode="postcode",
        ...
        Address_obj_street_city_state_postcode_country = Address.objects.create(
            street="street",
            city="city",
            state="state",
            postcode="postcode",
        ...
        Address_obj_street_address_line_state_postcode_country = Address.objects.create(
            street="street",
            address_line="address_line",
            state="state",
            postcode="postcode",
        ...
        Address_obj_city_address_line_state_postcode_country = Address.objects.create(
            city="city",
            address_line="address_line",
            state="state",
            postcode="postcode",
        ...
        self.assertEqual(
            Address_obj_address_line.get_complete_address(), "address_line"
        )
        ...
        self.assertEqual(
            Address_obj_street_address_line.get_complete_address(),
            "address_line, street",
        )
        self.assertEqual(
            Address_obj_street_state.get_complete_address(), "street, state"
        )
        self.assertEqual(
            Address_obj_street_postcode.get_complete_address(), "street, postcode"
        )
        self.assertEqual(
            Address_obj_street_country.get_complete_address(), "street, India"
        )
        self.assertEqual(
            Address_obj_city_address_line.get_complete_address(), "address_line, city"
        )
        self.assertEqual(Address_obj_city_state.get_complete_address(), "city, state")
        self.assertEqual(
            Address_obj_city_postcode.get_complete_address(), "city, postcode"
        )
        self.assertEqual(Address_obj_city_country.get_complete_address(), "city, India")
        self.assertEqual(
            Address_obj_address_line_state.get_complete_address(), "address_line, state"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            Address_obj_address_line_country.get_complete_address(),
            "address_line, India",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            Address_obj_state_country.get_complete_address(), "state, India"
        )
        self.assertEqual(
            Address_obj_postcode_country.get_complete_address(), "postcode, India"
        )
        self.assertEqual(
            Address_obj_street_city_address_line.get_complete_address(),
            "address_line, street, city",
        )
        self.assertEqual(
            Address_obj_street_city_state.get_complete_address(), "street, city, state"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            Address_obj_street_city_country.get_complete_address(),
            "street, city, India",
        )
        self.assertEqual(
            Address_obj_street_address_line_state.get_complete_address(),
            "address_line, street, state",
        )
        self.assertEqual(
            Address_obj_street_address_line_postcode.get_complete_address(),
            "address_line, street, postcode",
        )
        self.assertEqual(
            Address_obj_street_address_line_country.get_complete_address(),
            "address_line, street, India",
        )
        self.assertEqual(
            Address_obj_street_state_postcode.get_complete_address(),
            "street, state, postcode",
        )
        self.assertEqual(
            Address_obj_street_state_country.get_complete_address(),
            "street, state, India",
        )
        self.assertEqual(
            Address_obj_street_postcode_country.get_complete_address(),
            "street, postcode, India",
        )
        self.assertEqual(
            Address_obj_city_address_line_state.get_complete_address(),
            "address_line, city, state",
        )
        self.assertEqual(
            Address_obj_city_address_line_postcode.get_complete_address(),
            "address_line, city, postcode",
        )
        self.assertEqual(
            Address_obj_city_address_line_country.get_complete_address(),
            "address_line, city, India",
        )
        self.assertEqual(
            Address_obj_city_state_postcode.get_complete_address(),
            "city, state, postcode",
        )
        self.assertEqual(
            Address_obj_city_state_country.get_complete_address(), "city, state, India"
        )
        self.assertEqual(
            Address_obj_city_postcode_country.get_complete_address(),
            "city, postcode, India",
        )
        self.assertEqual(
            Address_obj_address_line_state_postcode.get_complete_address(),
            "address_line, state, postcode",
        )
        self.assertEqual(
            Address_obj_address_line_state_country.get_complete_address(),
            "address_line, state, India",
        )
        self.assertEqual(
            Address_obj_address_line_postcode_country.get_complete_address(),
            "address_line, postcode, India",
        )
        self.assertEqual(
            Address_obj_state_postcode_country.get_complete_address(),
            "state, postcode, India",
        )
        self.assertEqual(
            Address_obj_street_city_address_line_state.get_complete_address(),
            "address_line, street, city, state",
        )
        self.assertEqual(
            Address_obj_street_city_address_line_postcode.get_complete_address(),
            "address_line, street, city, postcode",
        )
        self.assertEqual(
            Address_obj_street_city_address_line_country.get_complete_address(),
            "address_line, street, city, India",
        )
        self.assertEqual(
            Address_obj_street_city_state_postcode.get_complete_address(),
            "street, city, state, postcode",
        )
        self.assertEqual(
            Address_obj_street_city_state_country.get_complete_address(),
            "street, city, state, India",
        )
        self.assertEqual(
            Address_obj_street_city_postcode_country.get_complete_address(),
            "street, city, postcode, India",
        )
        self.assertEqual(
            Address_obj_street_address_line_state_postcode.get_complete_address(),
            "address_line, street, state, postcode",
        )
        self.assertEqual(
            Address_obj_street_address_line_state_country.get_complete_address(),
            "address_line, street, state, India",
        )
        self.assertEqual(
            Address_obj_street_address_line_postcode_country.get_complete_address(),
            "address_line, street, postcode, India",
        )
        self.assertEqual(
            Address_obj_street_state_postcode_country.get_complete_address(),
            "street, state, postcode, India",
        )
        self.assertEqual(
            Address_obj_city_address_line_state_postcode.get_complete_address(),
            "address_line, city, state, postcode",
        )
        self.assertEqual(
            Address_obj_city_address_line_state_country.get_complete_address(),
            "address_line, city, state, India",
        )
        self.assertEqual(
            Address_obj_city_address_line_postcode_country.get_complete_address(),
            "address_line, city, postcode, India",
        )
        self.assertEqual(
            Address_obj_city_state_postcode_country.get_complete_address(),
            "city, state, postcode, India",
        )
        self.assertEqual(
            Address_obj_address_line_state_postcode_country.get_complete_address(),
            "address_line, state, postcode, India",
        )
        self.assertEqual(
            Address_obj_street_city_address_line_state_postcode.get_complete_address(),
            "address_line, street, city, state, postcode",
        )
        self.assertEqual(
            Address_obj_street_city_address_line_state_country.get_complete_address(),
            "address_line, street, city, state, India",
        )
        self.assertEqual(
            Address_obj_street_city_address_line_postcode_country.get_complete_address(),
            "address_line, street, city, postcode, India",
        )
        self.assertEqual(
            Address_obj_street_city_state_postcode_country.get_complete_address(),
            "street, city, state, postcode, India",
        )
        self.assertEqual(
            Address_obj_street_address_line_state_postcode_country.get_complete_address(),
            "address_line, street, state, postcode, India",
        )
        self.assertEqual(
            Address_obj_city_address_line_state_postcode_country.get_complete_address(),
            "address_line, city, state, postcode, India",
        )
        self.assertEqual(
            Address_obj_street_city_address_line_state_postcode_country.get_complete_address(),
            "address_line, street, city, state, postcode, India",
        )
    ...
    def test_file_extensions_classes(self):
        doc_mp3 = Attachments.objects.create(
            file_name="file_mp3", attachment="file.mp3"
        )
        doc_mp4 = Attachments.objects.create(
            file_name="file_mp4", attachment="file.mp4"
        )
        doc_png = Attachments.objects.create(
            file_name="file_png", attachment="file.png"
        )
        doc_pdf = Attachments.objects.create(
            file_name="file_pdf", attachment="file.pdf"
        )
        doc_json = Attachments.objects.create(
            file_name="file_json", attachment="file.json"
        )
        doc_txt = Attachments.objects.create(
            file_name="file_txt", attachment="file.txt"
        )
        doc_csv = Attachments.objects.create(
            file_name="file_csv", attachment="file.csv"
        )
        doc_zip = Attachments.objects.create(
            file_name="file_zip", attachment="file.zip"
        )
        doc_unknown_extension = Attachments.objects.create(
            file_name="file_unknown_extension", attachment="file.unknown_extension"
        )
        doc_without_extension = Attachments.objects.create(
            file_name="file_unknown_extension", attachment="file_without_extension"
        )
        doc_without_attachment = Attachments.objects.create(
            file_name="file_unknown_extension"
        )
    ...
    def test_arrow_format(self):
        ...
        self.comment = Comment.objects.create(
            comment="comment", user=self.user, commented_by=self.user, commented_on=date
        )
