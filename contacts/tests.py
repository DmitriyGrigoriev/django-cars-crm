...
class ContactObjectsCreation(object):
    def setUp(self):
        ...
        self.attachment = Attachments.objects.create(
            attachment="image.png", case=self.case, created_by=self.user
        )
...
class ContactViewsTestCase(ContactObjectsCreation, TestCase):
    ...
    def test_contacts_delete_get(self):
        response = self.client.get("/contacts/" + str(self.contact.id) + "/delete/")
        self.assertEqual(response.status_code, 302)
    ...
    def test_contacts_edit_post(self):
        response = self.client.get("/contacts/" + str(self.contact.id) + "/edit/")
        self.assertEqual(response.status_code, 200)
...
class ContactsListTestCase(ContactObjectsCreation, TestCase):
    ...
    def test_contacts_list_user_role(self):
        ...
        response = self.client.post(
            ...
            {"first_name": "john contact", "assigned_to": str(self.user.id)},
        )
        self.assertEqual(response.status_code, 200)
...
class CommentTestCase(ContactObjectsCreation, TestCase):
    ...
    def test_form_valid(self):
        response = self.client.post(
            "/contacts/comment/add/",
            {"contactid": self.contact.id, "comment": "test comment"},
        )
...
class TestContactCreateContact(ContactObjectsCreation, TestCase):
    ...
    def test_contact_update_view_error(self):
        ...
        response = self.client.post(
            reverse("contacts:remove_contact", args=(self.contact.id,)),
            {"pk": self.contact.id},
        )
        self.assertEqual(403, response.status_code)
        self.contact.delete()
...
class TestContactViews(ContactObjectsCreation, TestCase):
    def test_create_contact(self):
        ...
        response = self.client.post(
            ...
            + "?view_account={}&address_form=".format(self.account_by_user.id),
            data,
        )
        ...
        response = self.client.post(
            reverse("contacts:add_contact")
            + "?view_account={}".format(self.account_by_user.id),
            data,
        ...
        response = self.client.get(
            reverse("contacts:view_contact", args=(self.contact.id,))
        )
        self.assertEqual(403, response.status_code)
        ...
        response = self.client.post(
            reverse("contacts:edit_contact", args=(self.contact.id,))
            + "?from_account={}".format(self.account_by_user.id),
            data,
        )
        self.assertEqual(302, response.status_code)
        response = self.client.post(
            reverse("contacts:edit_contact", args=(self.contact.id,))
            + "?from_account={}&address_form=".format(self.account_by_user.id),
            data,
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        ...
        response = self.client.post(
            reverse("contacts:remove_contact", args=(self.contact_delete.id,)),
        ...
        self.client.login(username="joeUser@contact.com", password="password")
        response = self.client.get(
            reverse("contacts:view_contact", args=(self.contact_edit.id,)),
