...
class ContactsListView(SalesAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if (
            ...
            or self.request.POST.get("phone")
...
class ContactDetailView(SalesAccessRequiredMixin, LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        contact_accounts = set(
            context["object"].account_contacts.values_list("id", flat=True)
        )
...
class UpdateContactView(SalesAccessRequiredMixin, LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        assigned_to_ids = (
            self.get_object().assigned_to.all().values_list("id", flat=True)
        )
...
class AddCommentView(LoginRequiredMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            request.user in self.contact.assigned_to.all()
            or request.user == self.contact.created_by
