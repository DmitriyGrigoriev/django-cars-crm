...
from common.models import APISettings, Attachments, Comment, User, Company
...
class TestLeadModel(object):
    def setUp(self):
        ...
        self.lead1 = Lead.objects.create(title="jane doe lead", created_by=self.user3)
...
class LeadsPostrequestTestCase(TestLeadModel, TestCase):
    def test_valid_postrequesttestcase_date(self):
        data = {
            ...
            "first_name": "john doe",
            "last_name": "doe",
            "email": "johnDoeLeadtitle@example.com",
            "account": self.account,
            "address_line": "",
            "street": "street",
            "city": "city name",
...
class CommentTestCase(TestLeadModel, TestCase):
    def test_comment_add(self):
        ...
        response = self.client.post("/leads/comment/add/", {"leadid": self.lead.id})
        self.assertEqual(response.status_code, 200)
...
class AttachmentTestCase(TestLeadModel, TestCase):
    def test_attachment_add(self):
        self.client.login(email="janeLead1@example.com", password="password")
        response = self.client.post("/leads/attachment/add/", {"leadid": self.lead.id})
        self.assertEqual(response.status_code, 200)
...
class TestLeadListView(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create(
            username="johnDoeG@lead.com",
            email="johnDoeG@lead.com",
            has_sales_access=True,
            company=self.company,
        )
        self.user.set_password("password")
    ...
    def test_lead_list_view(self):
        ...
        # self.assertEqual(force_str(response.content), {"errors": {"title": ["This field is required."]}, "error": True})


class TestUpdateLeadView(TestLeadModel, TestCase):
    def test_lead_update_view(self):
        response = self.client.post(
            reverse("leads:edit_lead", args=(self.lead.id,)),
...
class TestCommentAddResponse(TestLeadModel, TestCase):
    def test_comment_add_response(self):
        ...
        response = self.client.post("/leads/attachment/add/", {"leadid": self.lead1.id})
        self.assertJSONEqual(
            force_str(response.content), {"error": ["This field is required."]}
        )
    ...
    def test_file_upload_for_leads(self):
        ...
        file_content_missing_headers = bytes(
            "".join(file_content_missing_headers), "utf-8"
        )
        data = {
            "leads_file": SimpleUploadedFile("file name", file_content_missing_headers)
        }
        response = self.client.post(reverse("leads:upload_lead_csv_file"), data)
        ...
        self.lead_user = Lead.objects.create(
            title="jane doe lead 2", created_by=self.user2
        )
        response = self.client.get(
            reverse("leads:view_lead", args=(self.lead_user.id,))
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.client.login(email="johnLead@example.com", password="password")
        response = self.client.get(
            reverse("leads:edit_lead", args=(self.lead_1.id,)) + "?status=converted"
        )
        self.assertEqual(response.status_code, 200)

        response = self.client.get(
            reverse("leads:edit_lead", args=(self.lead_1.id,)) + "?status=c"
        )
        self.assertEqual(response.status_code, 200)
        ...
        comment = Comment.objects.create(
            comment="comment text lead", lead=self.lead_1, commented_by=self.user
        )
        response = self.client.post(
            reverse("leads:edit_lead", args=(self.lead_1.id,)) + "?status=converted",
            data,
        ...
        self.lead.save()
        response = self.client.get(
            reverse("leads:leads_convert", args=(self.lead.id,)) + "?status=converted"
        )
        self.assertEqual(response.status_code, 302)

        self.tag_for_lead = Tags.objects.create(name="tag for lead")
        data = {"tags": "tag1 lead,tag2 lead,tag for lead"}
        response = self.client.post(
            reverse("leads:update_lead_tags", args=(self.lead.id,)), data
        )
        self.assertEqual(response.status_code, 302)
        ...
        self.client.logout()
        self.client.login(email="johnLead@example.com", password="password")
        data = {"lead": self.lead.id, "tag": self.tag_for_lead.id}
