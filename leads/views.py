...
from leads.forms import LeadAttachmentForm, LeadCommentForm, LeadForm, LeadListForm
...
def lead_list_view(request):
    ...
    if request.method == "GET":
        ...
        context["users"] = (
            User.objects.filter(is_active=True, company=request.company)
            .order_by("username")
            .values("id", "username")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        search = (
            ...
            if (
                ...
                or self.request.POST.get("source")
...
def create_lead(request):
    ...
    form = LeadForm(assigned_to=users, request_obj=request)

    if request.POST:
        form = LeadForm(
            request.POST, request.FILES, assigned_to=users, request_obj=request
        )
        if form.is_valid():
            lead_obj = form.save(commit=False)
            lead_obj.created_by = request.user
            lead_obj.company = request.company
            lead_obj.save()
...
class LeadDetailView(SalesAccessRequiredMixin, LoginRequiredMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if contact.company != request.company:
            raise PermissionDenied
        return super(LeadDetailView, self).dispatch(request, *args, **kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        attachments = Attachments.objects.filter(lead__id=self.object.id).order_by(
            "-id"
        )
...
def update_lead(request, pk):
    ...
    if lead_record.company != request.company:
        raise PermissionDenied
        ...
        if (
            ...
            if request.GET.get("page"):
                return HttpResponseRedirect(
                    reverse("leads:list") + "?page=" + request.GET.get("page")
                )
...
class AddCommentView(LoginRequiredMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            request.user in self.lead.assigned_to.all()
            or request.user == self.lead.created_by
