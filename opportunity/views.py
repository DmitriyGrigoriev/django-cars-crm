...
class OpportunityListView(SalesAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if (
            ...
            or self.request.POST.get("stage")
            or self.request.POST.get("lead_source")
            or self.request.POST.get("account")
            or self.request.POST.get("contacts")
        ):
            search = True
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if request.POST.getlist("assigned_to", []):
                opportunity_obj.assigned_to.add(*request.POST.getlist("assigned_to"))
            ...
            if request.POST.getlist("teams", []):
                ...
                assinged_to_users_ids = opportunity_obj.assigned_to.all().values_list(
                    "id", flat=True
                )
            ...
            if request.FILES.get("oppurtunity_attachment"):
                ...
                attachment.file_name = request.FILES.get("oppurtunity_attachment").name
                attachment.opportunity = opportunity_obj
                attachment.attachment = request.FILES.get("oppurtunity_attachment")
                attachment.save()
...
class OpportunityDetailView(SalesAccessRequiredMixin, LoginRequiredMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if opportunity.company != request.company:
            raise PermissionDenied
        return super(OpportunityDetailView, self).dispatch(request, *args, **kwargs)
...
def update_opportunity(request, pk):
    ...
    if request.POST:
        form = form = OpportunityForm(
            request.POST, request.FILES, instance=opportunity_object, **kwargs_data
        )
        if form.is_valid():
...
class AddCommentView(LoginRequiredMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            request.user in self.opportunity.assigned_to.all()
            or request.user == self.opportunity.created_by
