...
def invoices_list(request):
    ...
    if request.method == "GET":
        ...
        context["invoices"] = invoices.filter(org=request.user.profile_set.get(user=request.user).org).order_by(
            "-created_on"
        )
        context["status"] = status
        context["users"] = users.filter(profile=request.user.profile_set.filter(org=request.user.profile_set.get(user=request.user).org))
        #users.filter(company=request.company)
